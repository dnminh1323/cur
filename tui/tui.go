...
func Run() (productIndexSelected string) {
	...
	fmt.Printf(green, Trr.Tr("CURSOR VIP PRO")+` v`+strings.Join(strings.Split(fmt.Sprint(version), ""), "."))
	Cli.SetProxy(lang)
	sCount, sPayCount, _, _, exp := Cli.GetMyInfo(deviceID)
	fmt.Printf(green, Trr.Tr("DeviceID")+":"+deviceID)
	expTime, _ := time.ParseInLocation("2006-01-02 15:04:05", exp, time.Local)
	// fmt.Printf(green, Trr.Tr("Paid expiration time")+":"+exp)
	// fmt.Printf("\033[32m%s\033[0m\u001B[1;32m %s \u001B[0m\033[32m%s\033[0m\u001B[1;32m %s \u001B[0m\u001B[32m%s\u001B[0m\n",
	// 	Trr.Tr("Promotion command:(promoted "), sCount, Trr.Tr(" paid"), sPayCount, Trr.Tr("every 10 people promoted or 2 people paid can get a one-year license)"))

	fmt.Printf(hGreen, "bash <(curl "+githubPath+"install.sh) "+deviceID+"\n")
	// chỗ này in ra dòng: bash <(curl -Lk https://ghp.ci/https://github.com/kingparks/cursor-vip/releases/download/latest/install.sh) f8de112ceb6d26860461834876392a47

	// printAD()
	// checkUpdate(version)
	// fmt.Println()
	// if len(jbProduct) > 1 {
	// 	fmt.Printf(defaultColor, Trr.Tr("Choose the product to authorize: "))
	// 	for i, v := range jbProduct {
	// 		fmt.Printf(hGreen, fmt.Sprintf("%d. %s\t", i+1, v))
	// 	}
	// 	fmt.Println()
	// 	fmt.Print(Trr.Tr("Please enter the product number (press Enter directly to default to 1, you can enter multiple at the same time, such as 145): "))
	// 	productIndex := 1
	// 	_, _ = fmt.Scanln(&productIndex)
	// 	if productIndex < 1 {
	// 		fmt.Println(Trr.Tr("Input error"))
	// 		return
	// 	}
	...
	// 	if errClip == nil {
	// 		isCopyText = Trr.Tr("（Copied to clipboard）")
	// 	}
	...
	// 	fmt.Println()
	// }
	fmt.Printf(green, Trr.Tr("Authorization successful! Please do not close this window during use"))
	return
}
...
func checkUpdate(version int) {
	...
	if upUrl != "" {
		fmt.Printf(red, Trr.Tr("There is a new version available, trying to update automatically. If it fails, please enter the following command and press Enter to update the program manually:"))
		fmt.Println()
		...
		fmt.Println(Trr.Tr("Update completed, rerun the program to take effect"))
		os.Exit(0)
		return
	}
}
